...
| pi-trajectory-inspector | `shared/ui/` | Trajectory Cockpit 检视器:Summary/Payload/Result/Schema/Timing;大 payload 只在此挂载;Schema 待 Gateway 解析能力 [#107](https://github.com/BubblePtr/pace/issues/107)(现为 unavailable 诚实态)。tintinweb `Agent` 步骤可带 “Open child session”（子 JSONL 未索引时禁用） |
